...
function JobDetails(): React.JSX.Element {
  ...
  const handleApply = async (): Promise<void> => {
    ...
    try {
      ...
      setApplyStatus({ type: 'success', message: 'Successfully applied for this job!' });
    } catch (err) {
  ...
  return (
    <div className="bg-white p-8 rounded-lg shadow-lg max-w-4xl mx-auto">
      <div className="flex items-start space-x-6 mb-6">
        <img src={job.logo} alt={`${job.company} logo`} className="w-24 h-24 object-contain rounded-md border" />
        <div>
      ...
      <div className="mt-6">
        <h2 className="text-xl font-semibold mb-2 text-gray-800">Required Skills</h2>
        <div className="flex flex-wrap gap-2">
          {job.skills.map(skill => (
            <span key={skill} className="bg-primary-100 text-primary-800 text-sm font-semibold px-3 py-1 rounded-full">{skill}</span>
          ))}
        </div>
      </div>
      
      <div className="mt-8">
        {applyStatus && <Alert type={applyStatus.type} message={applyStatus.message} onClose={() => setApplyStatus(null)} />}
      </div>
      ...
      <div className="mt-8 border-t pt-6">
        {isAuthenticated ? (
          ...
          >
            {isApplying ? 'Applying...' : applyStatus?.type === 'success' ? 'Applied' : 'Apply Now'}
          </button>
        ) : (
          <p className="text-center bg-yellow-100 p-4 rounded-md">
            <Link to="/login" className="text-primary-600 font-bold hover:underline">Log in</Link> to apply for this job.
          </p>
        )}
      </div>
    </div>
