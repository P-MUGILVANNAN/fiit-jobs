...
function Navbar(): React.JSX.Element {
  ...
  const baseLinkClasses =
    "transition-all duration-200 rounded-lg flex items-center gap-2 font-semibold";
  ...
  return (
    <nav className="bg-white shadow-lg sticky top-0 z-50">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          ...
          <div className="flex-shrink-0 flex items-center">
            <Link
              to="/"
              className="text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-purple-600 flex items-center gap-2"
            >
              <Rocket size={32} />
              FIIT JOBS
            </Link>
          ...
          <div className="hidden lg:block">
            <div className="flex items-center space-x-6">
              ...
              <NavLink to="/resume" className={navLinkClasses}><FileText size={18} />Resume</NavLink>
              {isAuthenticated && (
          ...
          <div className="hidden lg:block relative">
            {isAuthenticated ? (
              <div className="flex items-center space-x-4 relative">
                ...
                <div className="relative">
                  <button
                    ...
                    className="text-gray-700 flex items-center gap-3 cursor-pointer select-none px-3 py-2 rounded-full hover:bg-gray-100 transition-colors focus:outline-none"
                    aria-expanded={isDropdownOpen}
                    aria-haspopup="true"
                  >
                    <div className="w-9 h-9 rounded-full overflow-hidden border-2 border-blue-500">
                      <img
                        src={user?.profileImage || "https://placehold.co/100x100/A78BFA/ffffff?text=U"}
                        alt="Profile"
                    ...
                    <ChevronDown
                      size={18}
                      className={`ml-1 transition-transform duration-200 ${isDropdownOpen ? 'rotate-180' : 'rotate-0'}`}
                    />
                  </button>

                  <div
                    className={`absolute top-full mt-2 right-0 w-56 bg-white border border-gray-200 rounded-lg shadow-xl
                  ...
                  >
                    <Link
                      ...
                      className="flex items-center gap-3 px-5 py-3 text-gray-700 font-semibold hover:bg-purple-100 hover:text-purple-700 transition-colors rounded-lg"
                    >
              ...
              <div className="flex items-center space-x-2">
                <Link
                  to="/login"
                  className="text-blue-600 hover:text-blue-800 px-5 py-2 text-sm font-semibold transition-colors flex items-center gap-2"
                >
                  <LogIn size={16} />Login
                </Link>
                <Link
                  to="/register"
                  className="bg-blue-600 text-white px-5 py-2 rounded-full text-sm font-semibold hover:bg-blue-700 transition-colors shadow-lg flex items-center gap-2"
                >
          ...
          <div className="-mr-2 flex lg:hidden">
            ...
            >
              {isOpen ? <X className="block h-8 w-8 text-gray-800" /> : <Menu className="block h-8 w-8 text-gray-800" />}
            </button>
          </div>
        </div>
      </div>
      ...
      {/* --- Mobile Menu --- */}
      <div
        className={`transition-all duration-300 ease-in-out overflow-hidden lg:hidden ${isOpen ? 'max-h-screen opacity-100' : 'max-h-0 opacity-0'}`}
        id="mobile-menu"
      >
        <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
          ...
          <NavLink to="/resume" className={mobileNavLinkClasses} onClick={() => setIsOpen(false)}><FileText size={20} />Resume</NavLink>
          {isAuthenticated && (
          ...
          {isAuthenticated ? (
            <div className="space-y-2">
              <div className="text-gray-700 text-lg mb-2 flex items-center gap-2">
                ...
                <span>Hello, <span className="font-bold uppercase">{user?.name || 'name'}</span></span>
              </div>
              <Link
                to="/profile"
                onClick={() => setIsOpen(false)}
                className="w-full text-center bg-gray-100 text-gray-800 px-4 py-3 rounded-full text-base font-semibold hover:bg-purple-100 hover:text-purple-700 transition-colors flex items-center justify-center gap-2"
              >
